...
class PhysicalL3RouterApplianceDBMixin(l3_router_appliance_db.L3RouterApplianceDBMixin,
    ...
    def sync_asr_list_with_db(self, context, asr_list):
        ...
        phy_router_qry = context.session.query(l3_models.CiscoPhysicalRouter).all()
        ...
        with context.session.begin(subtransactions=True):
            ...
            for asr in asr_list:
                if asr['name'] not in db_names:
                    new_db_asr = l3_models.CiscoPhysicalRouter(name=asr['name'])
    ...
    def _delete_hsrp_interfaces(self, context, router_id, subnet, dev_owner):
        ...
        for port in port_list:
            ...
            ha_info = {'id': router_id,
                       ...
                       'subnet_id': subnet['id']}
            notifier = n_rpc.get_notifier('network')
            router_event = 'router.interface.delete'
            notifier.info(context, router_event,
                      {'router_interface': ha_info})
    ...
    def _bind_hsrp_interfaces_to_router(self, context, router_id,  port_list):
        ...
        for db_asr, port in zip(phy_router_qry, port_list):            
            port_binding = l3_models.CiscoPhyRouterPortBinding(port_id=port['id'],
                                                               subnet_id=port['fixed_ips'][0]['subnet_id'],
                                                               router_id=router_id,
    ...
    def get_sync_data(self, context, router_ids=None, active=None):
        ...
        with context.session.begin(subtransactions=True):
            ...
            for ha_intf in all_ha_interfaces:
                port_id = ha_intf['id']
                phy_port_qry = context.session.query(l3_models.CiscoPhyRouterPortBinding,
                                                     l3_models.CiscoPhysicalRouter)
                phy_port_qry = phy_port_qry.filter(l3_models.CiscoPhyRouterPortBinding.port_id == port_id)
                phy_port_qry = phy_port_qry.filter(l3_models.CiscoPhyRouterPortBinding.phy_router_id == \
                                                   l3_models.CiscoPhysicalRouter.id)
                try:
