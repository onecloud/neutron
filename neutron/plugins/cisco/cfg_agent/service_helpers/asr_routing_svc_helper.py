...
class PhyRouterContext(routing_svc_helper.RoutingServiceHelper):
    ...
    def connection_err_callback(self, ex):
        LOG.exception(_("NetConf connection exception: %s"), ex)
        self.fullsync = True
    ...
    def process_service(self, device_ids=None, removed_devices_info=None):
        try:
            LOG.info(_("Sending heartbeat to ASR"))
        ...
        try:
            ...
            for r in routers:
                ...
                try:
                    ...
                    self.updated_routers.update([r['id']])
                    # TODO(onecloud): Do fullsync on error to
                    # be safe for now, can optimize later
                    self.fullsync = True
                    continue
    ...
    def _process_router(self, ri):
        ...
        try:
            ...
            old_ports, new_ports = \
                self._get_port_set_diffs(ri.internal_ports, internal_ports)
    ...
    def _process_router_floating_ips(self, ri, ex_gw_port):
        ...
        LOG.debug(_("existing_floating_ip: %(existing_floating_ip_ids)s, "
                    "cur_floating_ip:%(cur_floating_ip_ids)s"),
                  {'existing_floating_ip_ids': existing_floating_ip_ids,
        ...
        for fip in floating_ips:
            if fip['port_id']:
                ...
                if fip['id'] not in existing_floating_ip_ids:
                    ...
                    # same loop cycle
                    # If add occurs before first, cfg will fail because of
                    # existing entry with identical fixed_ip
                    fips_to_add.append(fip)

        floating_ip_ids_to_remove = (existing_floating_ip_ids -
                                     cur_floating_ip_ids)

        LOG.debug(_("fip_ids_to_remove: %s"), floating_ip_ids_to_remove)
        ...
        for fip in ri.floating_ips:
            if fip['id'] in floating_ip_ids_to_remove:
                ...
                LOG.debug(_("FIP: %(new_fip)s, "
                            "old FxIP: %(existing_fixed_ip)s, "
                            "new FxIP: %(new_fixed_ip)s\n\n"),
                          {'new_fip': new_fip,
                           'existing_fixed_ip': existing_fixed_ip,
                           'new_fixed_ip': new_fixed_ip})
...
class RoutingServiceHelperWithPhyContext(
    ...
    def __init__(self, host, conf, cfg_agent):
        ...
        for asr in self._asr_config.get_asr_list():
            self._asr_contexts[asr['name']] = \
                PhyRouterContext(asr, self.plugin_rpc, self.context,
                                 self._dev_status)

    ### Notifications from Plugin ####
    def router_deleted(self, context, routers):
        """Deal with router deletion RPC message."""
        LOG.debug(_('Got router deleted notification for %s'), routers)
    ...
    def process_service(self, device_ids=None, removed_devices_info=None):
        try:
            ...
            LOG.exception(_("Server heartbeat timeout"))
            self.resync_asrs(self.context)
            # don't try to configure ASRs, can't get latest DB info
            return
