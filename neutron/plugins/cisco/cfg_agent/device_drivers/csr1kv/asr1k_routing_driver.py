...
class ASR1kConfigInfo(object):
    ...
    def _create_asr_device_dictionary(self):
        ...
        LOG.info(_("ASR dict: %s"), self.asr_dict)
        LOG.info(_("Deployment IDs mine: %(deploy_id)s others: %(dep_id)s"),
                 {'deploy_id': self.deployment_id,
                  'dep_id': self.other_dep_ids})
...
class ASR1kRoutingDriver(csr1kv_driver.CSR1kvRoutingDriver):
    ...
    def internal_network_added(self, ri, port):
        ...
        if self._is_port_v6(port):
            LOG.debug(_("ADDING IPV6 NETWORK port: %s"), port)
    ...
    def external_gateway_added(self, ri, ex_gw_port):
        ...
        # tenant router handles interface creation and default
        # route within VRFs
        if self._is_global_router(ri):
            ex_gw_ip = ex_gw_port['subnet']['gateway_ip']
            virtual_gw_port = \
                self._get_virtual_gw_port_for_ext_net(ri, ex_gw_port)
            ...
            if self._fullsync and int(vlan_id) in \
                ...
                LOG.info(_("Subinterface already exists,"
                           "don't create interface"))
            else:
    ...
    def _csr_create_subinterface_v6(self, ri, port, is_external=False,
                                    gw_ip=""):
        ...
        # Start building the XML confstr
        ...
        confstr += asr_snippets.CREATE_SUBINTERFACE_V6_WITH_ID % (
            self._asr_config.deployment_id, vlan, ip_cidr, DEFAULT_IPV6_MTU
        )
        confstr += self._get_asr_ipv6_ra_mode_confstr(ra_mode, prefix)

        confstr += self._get_asr_hsrp_v6_confstr(ri, port, ip_cidr,
                                                 is_external)
    ...
    def _get_asr_hsrp_v6_confstr(self, ri, port, ip, is_external=False):
        if not self._v6_port_needs_config(port):
            return

        self._get_interface_vlan_from_hosting_port(port)
        # group = vlan
        if is_external:
    ...
    def _get_asr_ipv6_ra_mode_confstr(self, ra_mode, prefix):
        confstr = ""
        if ra_mode == constants.DHCPV6_STATEFUL:
            confstr = asr_snippets.SET_INTF_V6_STATEFUL % (prefix)
        elif ra_mode == constants.DHCPV6_STATELESS:
            confstr = asr_snippets.SET_INTF_V6_STATELESS
    ...
    def _asr_set_v6_tenant_route_from_port(self, ri, port, is_delete):
        ...
        subinterface = self._get_interface_name_from_hosting_port(port)
        prefix = port['subnet']['cidr']
        self._asr_set_v6_tenant_route(prefix, subinterface, vrf_name,
                                      is_delete)

    def _asr_set_v6_tenant_route(self, prefix, subinterface_name, vrf_name,
                                 is_delete):
        if is_delete:
            confstr = asr_snippets.REMOVE_V6_TENANT_NETWORK_ROUTE % \
                      (prefix, subinterface_name, vrf_name)
            self._edit_running_config(confstr,
                                      '%s REMOVE_V6_TENANT_NETWORK_ROUTE' %
                                      self.target_asr['name'])
        else:
            confstr = asr_snippets.ADD_V6_TENANT_NETWORK_ROUTE % \
                      (prefix, subinterface_name, vrf_name)
            self._edit_running_config(confstr,
                                      '%s ADD_V6_TENANT_NETWORK_ROUTE' %
                                      self.target_asr['name'])
    ...
    def _csr_remove_internalnw_nat_rules(self, ri, ports, ex_port,
        ...
        if self._is_port_v6(ex_port):
            LOG.debug(_("IPv6 port, no NAT delete needed"))
            return
        ...
        for port in ports:
            ...
            vrf_name = self._csr_get_vrf_name(ri)
            ext_intfc_name = \
                self._get_interface_name_from_hosting_port(ex_port)
            for acl in acls:
        ...
        try:
            if is_delete:
                ...
                self._edit_running_config(
                    confstr, '%s DELETE_NAT_POOL' % self.target_asr['name']
                )
            else:
                confstr = asr_snippets.CREATE_NAT_POOL % (pool_name,
                                                      ...
                                                      pool_net.netmask)
                self._edit_running_config(
                    confstr, '%s CREATE_NAT_POOL' % self.target_asr['name'])
        except cfg_exc.CSR1kvConfigException as cse:
            LOG.error(_("temporary disable NAT_POOL exception handling: %s"),
                      cse)

    def _create_subinterface_v6(self, subinterface, vlan_id, vrf_name, ip_cidr,
                                is_external=False):
        if is_external is True:
            confstr = asr_snippets.CREATE_SUBINTERFACE_V6_NO_VRF_WITH_ID % \
                      (subinterface, self._asr_config.deployment_id, vlan_id,
                       ip_cidr, DEFAULT_IPV6_MTU)
        else:
            confstr = asr_snippets.CREATE_SUBINTERFACE_V6_WITH_ID % \
                      (subinterface, self._asr_config.deployment_id, vlan_id,
                       vrf_name, ip_cidr, DEFAULT_IPV6_MTU)
    ...
    def _add_default_static_route_v6(self, gw_ip, vrf, out_intf):
        conn = self._get_connection()
        confstr = asr_snippets.SET_DEFAULT_ROUTE_V6_WITH_INTF % (vrf, out_intf,
                                                                 gw_ip)
        rpc_obj = conn.edit_config(target='running', config=confstr)
    ...
    def _create_ext_subinterface_enable_only(self, subinterface):
        ...
        self._edit_running_config(confstr, '%s ENABLE_INTF' %
                                  self.target_asr['name'])
    ...
    def _create_subinterface(self, subinterface, vlan_id, vrf_name, ip, mask,
        ...
        self._edit_running_config(confstr, '%s CREATE_SUBINTERFACE' %
                                  self.target_asr['name'])

    def _remove_subinterface(self, subinterface):
        confstr = snippets.REMOVE_SUBINTERFACE % subinterface
        self._edit_running_config(confstr, '%s REMOVE_SUBINTERFACE' %
                                  self.target_asr['name'])
    ...
    def _nat_rules_for_internet_access(self, acl_no, network,
        ...
        if self._fullsync and int(in_vlan) in \
            ...
            rpc_obj = conn.edit_config(target='running', config=confstr)
            self._check_response(rpc_obj, '%s SET_NAT_INSIDE' %
                                 self.target_asr['name'])

        if self._fullsync and int(out_vlan) in \
                self._existing_cfg_dict['interfaces']:
            LOG.debug(_("Skip cfg for existing 'nat outside'"))
            pass
        else:
    ...
    def _add_interface_nat(self, intfc_name, intfc_type):
        ...
        self._check_response(rpc_obj,
                             '%s SET_NAT %s' %
                             (self.target_asr_name, intfc_type))
    ...
    def _remove_dyn_nat_rule(self, acl_no, outer_intfc_name, vrf_name):
        ...
        rpc_obj = conn.edit_config(target='running', config=confstr)
        self._check_response(rpc_obj,
                             '%s REMOVE_ACL' % self.target_asr['name'])
    ...
    def _add_floating_ip(self, floating_ip, fixed_ip, vrf,
        ...
        The vlan in turn is applied to the redundancy parameter for setting the
        IP NAT.
        """
        conn = self._get_connection()
    ...
    def _add_default_static_route(self, gw_ip, vrf, out_intf):
        conn = self._get_connection()
        confstr = \
            asr_snippets.SET_DEFAULT_ROUTE_WITH_INTF % (vrf, out_intf, gw_ip)
        rpc_obj = conn.edit_config(target='running', config=confstr)
        self._check_response(rpc_obj,
    ...
    def _create_vrf(self, vrf_name):
        try:
            ...
            if self._check_response(
                    rpc_obj, '%s CREATE_VRF' % self.target_asr['name']
            ):
